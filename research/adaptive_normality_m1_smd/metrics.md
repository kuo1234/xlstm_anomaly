...
For contextual score fusion only, each raw detector score is mapped to a training-normal empirical upper-tail severity using the first half of calibration scores: tail(s) = (1 + count(calibration score >= s))/(n + 1), severity = -log10(tail). The control-only fusion is the maximum severity over last-value, moving median, VAR(1), and xLSTMAD-R. The forecast-plus-control fusion is the maximum of the control fusion and xLSTM-F severity. This mapping is fixed without labels. It is not a learned detector.
