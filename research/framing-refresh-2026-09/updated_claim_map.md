...
| **C4.** The effect is replicated in both xLSTM and a capacity-matched LSTM | SUPPORTED for the common scalar schema | LD `+0.142966` (50/50 units, 5/5 seeds, 10/10 sources); SC both backbones survive O2 | Two backbones is not "generic across recurrent architectures"; no GRU, no SSM, no transformer control | No prior art either way | "The common-state effect is not xLSTM-specific within the two tested recurrent backbones: a capacity-matched LSTM shows an equal or larger increment." | "Generic across recurrent architectures." / "All recurrent models contain this information." | optional: one third backbone (GRU) at the same protocol |
...
| **E16** | "generic across recurrent architectures" / "generic recurrent evidence" | Only xLSTM and one capacity-matched LSTM were tested. Use "replicated in both xLSTM and a capacity-matched LSTM", or "the common-state effect is not xLSTM-specific within the two tested recurrent backbones". |
