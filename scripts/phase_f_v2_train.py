"""One fixed Phase F-v2 training condition in a fresh process.

No anomaly labels, test sources, probes, or test metrics are imported here.
"""
import argparse
import hashlib
import json
...
def main():
    parser=argparse.ArgumentParser(); parser.add_argument('architecture',choices=('xlstm','lstm')); parser.add_argument('seed',type=int)
    args=parser.parse_args(); start=time.perf_counter()
    environment=configure(); sealed=verify_sealed_inputs()
    gates=json.loads((REPORT/'mechanical_gates.json').read_text())
    if gates.get('status')!='PASS': raise SystemExit('F-v2 mechanical gates are not PASS; training is forbidden')
    if args.seed not in CONFIG['detector_seeds']: raise ValueError('Unregistered detector seed')
    # The order file is the already sealed F-v1 artifact; this is an integrity
    # check, never regeneration or selection.
    ...
    run_name=f'{args.architecture}_{args.seed}'; data_dir=ROOT/'data/phase_f_v2'/'runs'/run_name; report_dir=REPORT/'runs'/run_name
    data_dir.mkdir(parents=True,exist_ok=False); report_dir.mkdir(parents=True,exist_ok=False)
    train=load_windows('train'); validation=load_windows('validation')
    model=build(args.architecture,args.seed); model.train(); opt=optimizer(model)
    expected={k:(tuple(v) if k=='betas' else v) for k,v in CONFIG['optimizer'].items()}
    assert all(opt.defaults[k]==v for k,v in expected.items())
    ...
    with forbid_native_predict() as trap:
        for epoch in range(50):
            ...
            for batch_ids in ids.split(128):
                observations=train[batch_ids]; opt.zero_grad(set_to_none=True)
            ...
            if validation_mse<best:
                best=validation_mse; best_epoch=epoch+1
                torch.save(dict(model=cpu_state(model),epoch=best_epoch,validation_mse=best),data_dir/'best.pt')
            row=dict(epoch=epoch+1,train_mse=train_mse,validation_mse=validation_mse,
                training_order_sha256=hashlib.sha256(order.tobytes()).hexdigest(),
                optimizer_steps_this_epoch=316,train_windows=len(train),validation_windows=len(validation),
    ...
    torch.save(dict(model=final,optimizer=opt.state_dict(),epoch=50,
        rng={'torch':torch.get_rng_state(),'cuda':torch.cuda.get_rng_state_all(),'numpy':np.random.get_state(),'python':random.getstate()}),data_dir/'final.pt')
    ...
    for name,state in initial.items():
        if recurrent_name(name):
            deltas[name]=dict(initial_norm=float(state.norm()),best_delta_norm=float((best_payload['model'][name]-state).norm()),
                final_delta_norm=float((final[name]-state).norm()),best_changed=not torch.equal(best_payload['model'][name],state))
    ...
    result=dict(status='PASS' if post['status']=='PASS' else 'STOP_POST_TRAIN_PARITY',architecture=args.architecture,seed=args.seed,
        implementation_commit=__import__('subprocess').check_output(['rtk','git','rev-parse','HEAD'],text=True).strip(),
        environment=environment,sealed_inputs=sealed,initial_model_hash=initial_hash,final_model_hash=final_hash,best_model_hash=best_hash,
        artifacts=artifacts,training_order_file_sha256=orderinfo['sha256'],consumed_order_sha256=order_hasher.hexdigest(),
        optimizer_config=CONFIG['optimizer'],epochs=50,optimizer_steps=optimizer_steps,window_exposures=exposures,
        parameter_count=CONFIG['parameters'][args.architecture],selected_epoch=best_epoch,best_validation_mse=best,
        recurrent_changes=deltas,training_seconds=training_seconds,total_seconds=time.perf_counter()-start,
        peak_allocated_bytes=torch.cuda.max_memory_allocated(),peak_reserved_bytes=torch.cuda.max_memory_reserved(),
        cpu_peak_rss_kib=resource.getrusage(resource.RUSAGE_SELF).ru_maxrss,native_predict_calls=trap.call_count,
        labels_used=False,test_sources_used=False,probe_fitting=False,post_training_parity=post)
    write_json(report_dir/'manifest.json',result)
    print(json.dumps(dict(run=run_name,status=result['status'],selected_epoch=best_epoch,total_seconds=result['total_seconds'])),flush=True)
